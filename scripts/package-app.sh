...
bundle_identifier="${AGENT_DECK_BUNDLE_ID:-app.agentdeck.dev}"
...
python_bin="${AGENT_DECK_PYTHON:-python3}"
...
"$python_bin" "$brand_script"
"$python_bin" "$dmg_bg_script"
...
install_name_tool -add_rpath @loader_path/../Frameworks "$bundle_dir/Contents/MacOS/AgentDeckApp" 2>/dev/null || true
...
<dict>
    ...
    <string>${AGENT_DECK_EDDSA_PUBLIC_KEY:-3IF8txq9RRNanzE2FNhyGRcwhslTucCcJHpTkpxcgBQ=}</string>
</dict>
</plist>
EOF
