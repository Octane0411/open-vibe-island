// Agent Deck plugin for OpenCode
// Bridges OpenCode events to the Agent Deck desktop app via Unix socket.
// Install: copy to ~/.config/opencode/plugins/agent-deck.js
import { connect } from "net";
import { appendFileSync } from "fs";
import { homedir } from "os";

const DEBUG_LOG = "/tmp/agent-deck-opencode-debug.log";
function debugLog(msg) {
  try { appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] ${msg}\n`); } catch {}
}

const SOCKET_PATH =
  process.env.AGENT_DECK_SOCKET_PATH ||
  `${process.env.HOME || homedir()}/Library/Application Support/AgentDeck/bridge.sock`;
  ...
  return {
    "event": async ({ event }) => {
      try {
        ...
        // Fail open: if Agent Deck is unavailable, don't block OpenCode
      }
    },

    "shell.env": async (input, output) => {
      output.env.AGENT_DECK_ACTIVE = "1";
