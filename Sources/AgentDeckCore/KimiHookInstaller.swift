...
public struct KimiHookInstallerManifest: Equatable, Codable, Sendable {
    public static let fileName = "agent-deck-kimi-hooks-install.json"
...
/// Installs/uninstalls Agent Deck's managed `[[hooks]]` entries in
/// `~/.kimi/config.toml`.
///
...
public enum KimiHookInstaller {
    public static let markerComment = "# agent-deck: managed hook — do not edit"
    ...
    private static func blockMatchesManagedCommand(_ block: [String], managedCommand: String?) -> Bool {
        for line in block {
            ...
            if isLegacyAgentDeckHookCommand(value) {
                return true
            }
        }
        return false
    ...
    /// escape handling is unnecessary — Agent Deck only ever writes paths,
    ...
    private static func isLegacyAgentDeckHookCommand(_ command: String) -> Bool {
        ...
        return normalized.contains("agentdeckhooks")
            || normalized.contains("agentdeckhooks")
            || normalized.contains("agent-deck-bridge")
            || normalized.contains("agent-deck-bridge")
    }
