...
| Target | Role |
|---|---|
| **AgentDeckApp** | SwiftUI + AppKit shell — menu bar extra, overlay panel (notch/top-bar), control center, settings. Entry point: `AgentDeckApp.swift` with `AppModel` as the central `@Observable` state owner. |
| **AgentDeckCore** | Shared library — models (`AgentSession`, `AgentEvent`, `SessionState`), bridge transport (Unix socket IPC with JSON line protocol), hook models/installers, transcript discovery, session persistence/registry. |
| **AgentDeckHooks** | Lightweight CLI executable invoked by agent hooks. Reads hook payload from stdin, forwards to app bridge via Unix socket, writes blocking JSON to stdout only when island denies a `PreToolUse`. |
| **AgentDeckSetup** | Installer CLI for managing `~/.codex/config.toml` and `hooks.json`. |
...
Agent
  │  stdin: JSON payload
  ▼
AgentDeckHooks CLI  (--source codex | --source claude | ...)
  │  Unix socket
  ▼
